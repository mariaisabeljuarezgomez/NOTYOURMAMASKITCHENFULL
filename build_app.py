...
function attach(el) {{
    el.onmousedown = (e) => {{
        ...
        if (e.shiftKey) {{
            ...
            if (docV2.editorState.selectedElementIds.length > 1 && docV2.editorState.selectedElementIds.includes(el.id)) {{
                // Group drag initiated, keep array intact
            }} else {{
        ...
        document.onmousemove = (me) => {{
            ...
            if (docV2.editorState.selectedElementIds.length > 1) {{
                const dx = parseFloat(el.style.left) - dragStartElX;
                const dy = parseFloat(el.style.top) - dragStartElY;
                docV2.editorState.selectedElementIds.forEach(id => {{
                    if (id === el.id) return;
                    if (groupStartMap[id]) {{
                        const otherEl = document.getElementById(id);
                        if (otherEl) {{
                            otherEl.style.left = (groupStartMap[id].startX + dx) + 'px';
                            otherEl.style.top = (groupStartMap[id].startY + dy) + 'px';
                        }}
                    }}
                }});
            }}
...
function updateSelectionBar() {{
    ...
    if (multiIds.length > 1) {{
        console.log("Showing Multi-Select Bar");
        ...
        document.getElementById('text-style-ctrls').style.display = 'none';
        document.getElementById('geo-style-ctrls').style.display = 'none';
        ...
        if (!msBlock) {{
            ...
            div.innerHTML = `<button class="ctrl-btn" style="background:#c0392b; width:auto; padding:0 15px;" onclick="var ids=[...docV2.editorState.selectedElementIds]; ids.forEach(id=>{{selectedId=id; deleteEl();}}); docV2.editorState.selectedElementIds=[]; deselect();">🗑️ Delete All (<span id="ms-del-count"></span>)</button><button class="ctrl-btn" style="background:#2980b9; width:auto; padding:0 15px;" onclick="var ids=[...docV2.editorState.selectedElementIds]; ids.forEach(id=>{{selectedId=id; duplicateEl();}}); docV2.editorState.selectedElementIds=[]; deselect();">📋 Duplicate All (<span id="ms-dup-count"></span>)</button>`;
