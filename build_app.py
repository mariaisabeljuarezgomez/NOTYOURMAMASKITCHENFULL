...
<div id="selection-bar">
    ...
    <div id="bar-content" style="display:flex; align-items:center; gap:10px; flex-grow:1; overflow-x:auto; scrollbar-width:none;">
        ...
        <div id="bar-tab-arrange" style="display:none; align-items:center; gap:10px;">
            <button class="ctrl-btn" onclick="duplicateSelected()" title="Duplicate" data-help="Creates an exact copy of the selected element, offset by 20px">📋</button>
            <button class="ctrl-btn" style="background:#c0392b;" onclick="deleteSelected()" title="Delete Element" data-help="Permanently removes the selected element from the layout">🗑️</button>
...
function duplicateSelected() {{
    if (!selectedId) return;
    const orig = docV2.elements.find(e => e.id === selectedId);
    if (!orig) return;
    const copy = JSON.parse(JSON.stringify(orig));
    copy.id = orig.type + '_' + Date.now();
    copy.x = (orig.x || 0) + 20;
    copy.y = (orig.y || 0) + 20;
    copy.zIndex = (orig.zIndex || 10) + 1;
    docV2.elements.push(copy);
    pushUndo();
    render();
    selectById(copy.id);
